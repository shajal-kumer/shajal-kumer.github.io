	...
	function CrearChat() {
		$('#' + htmlDOM.idChat).append(
			'<div class="' +
				...
				'<span id="patientname">' +
				'</span>' +
				'</div><div class="panel-body client-panel-body"><ul class="chatpluginchat client-chatlist"></ul></div><div class="panel-footer"><div class="input-group"><input id="' +
...
function callChat(token, clientname) {
	htmlDOM.Room = token;
	htmlDOM.Nombre = clientname;
	document.getElementById('patientname').innerHTML = htmlDOM.Nombre;
...
function startup(patientdata) {
	if (patientdata.conversationtoken !== '') {
		setLocalStorage(
			'patientConversationToken',
			patientdata.conversationtoken
		);
		setLocalStorage('patientname', patientdata.clientname);

		callChat(
			getLocalStorage('patientConversationToken'),
			getLocalStorage('patientname')
		);
	}
}
