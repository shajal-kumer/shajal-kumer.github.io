...
var RTCMultiConnection = function(roomid, forceOptions) {
  ...
  function setHarkEvents(connection, streamEvent) {
    if (
      streamEvent.stream &&
      streamEvent.stream.getAudioTracks &&
      streamEvent.stream.getAudioTracks().length
    ) {
  ...
  function setSdpConstraints(config) {
    ...
    return (
      "Chrome" === DetectRTC.browser.name && DetectRTC.browser.version >= 60,
      "Firefox" === DetectRTC.browser.name &&
        DetectRTC.browser.version >= 54 &&
        (oldBrowser = !1),
      oldBrowser &&
        (sdpConstraints = {
  ...
  function PeerInitiator(config) {
    ...
    function createOfferOrAnswer(_method) {
      return window.enableAdapter
        ? void peer[_method](defaults.sdpConstraints).then(
            function(localSdp) {
              "Safari" !== DetectRTC.browser.name &&
                (localSdp.sdp = connection.processSdp(localSdp.sdp)),
    ...
    window.enableAdapter || (incomingStreamEvent = "addstream"),
      peer.addEventListener(
        ...
        function(event) {
          if (
            event &&
            ("track" === incomingStreamEvent &&
              (event.stream = event.streams[event.streams.length - 1]),
            !dontDuplicate[event.stream.id] ||
              "Safari" === DetectRTC.browser.name)
          ) {
            ...
            streamToShare
              ...
              : ((event.stream.isVideo = !!event.stream.getVideoTracks()
                  .length),
                (event.stream.isAudio = !event.stream.isVideo),
                (event.stream.isScreen = !1)),
              (event.stream.streamid = event.stream.id),
              ("Firefox" != DetectRTC.browser.name && event.stream.stop) ||
                (event.stream.stop = function() {
                  var streamEndedEvent = "ended";
                  "oninactive" in event.stream &&
  ...
  "undefined" == typeof MediaStream &&
    ...
    (window.iOSDefaultAudioOutputDevice =
      window.iOSDefaultAudioOutputDevice || "speaker"),
    "undefined" == typeof window.enableAdapter &&
      ("Firefox" === DetectRTC.browser.name &&
        DetectRTC.browser.version >= 54 &&
        (window.enableAdapter = !0),
      "Chrome" === DetectRTC.browser.name && DetectRTC.browser.version >= 60,
      "undefined" != typeof adapter &&
        adapter.browserDetails &&
        "string" == typeof adapter.browserDetails.browser &&
        (window.enableAdapter = !0)),
    window.enableAdapter ||
  ...
  !(function(connection) {
    ...
    (mPeer.onGettingLocalMedia = function(stream, callback) {
      ...
      (connection.processSdp = function(sdp) {
        return "Safari" === DetectRTC.browser.name
          ? sdp
            ...
            onGettingLocalMedia: function(stream) {
              ...
              videoConstraints &&
                ...
                stream.isScreen ||
                  ...
                  (stream.isAudio =
                    !stream.isVideo && stream.getAudioTracks().length)),
      ...
      (connection.replaceTrack = function(session, remoteUserId, isVideoTrack) {
        ...
        if (session instanceof MediaStream)
          return (
            session.getVideoTracks().length &&
              replaceTrack(session.getVideoTracks()[0], remoteUserId, !0),
            void (
              session.getAudioTracks().length &&
              replaceTrack(session.getAudioTracks()[0], remoteUserId, !1)
            )
          );
        if (isData(session))
